// generated - do not edit!
// generated by @xsip cs2 schema generator

#pragma once

#include <SDK/GlobalTypes.hpp>

#include <SDK/particles/CParticleFunctionRenderer.hpp>



namespace CS2 {
	namespace particles {
		class C_OP_RenderPoints : public CS2::particles::CParticleFunctionRenderer {
		public:
			// GlobalTypes::CStrongHandle< resourcesystem::InfoForResourceTypeIMaterial2 > m_hMaterial; // 0x210 | Schema_Atomic | Size: 0x8
			char  m_hMaterial[0x8]; // 0x210 | Schema_Atomic | Size: 0x8
		};
		static_assert(offsetof(CS2::particles::C_OP_RenderPoints, m_hMaterial) == 0x210, "m_hMaterial in C_OP_RenderPoints should be at offset 0x210");
		static_assert(sizeof(CS2::particles::C_OP_RenderPoints) == 0x218, "C_OP_RenderPoints size should be 0x218");
	}
}
